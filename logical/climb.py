# You are in some climbing competition. You start with some stamina which is a positive
# integer number. And you have obstacles in a list. Each number in list represents
# obstacle height.
#
# While climbing up you lose 2 stamina for up to 1 meter climbed.
# So if you climb 0.3m up you lose 2 stamina, if you climb 1m you lose 2 stamina,
# if you climb 1.8m you lose 4 stamina(2 for 1m and 2 for 0.8m) and so on.
#
# While climbing down you lose 1 stamina for up to 1 meter climbed. So if you climb 0.5m you
# lose 1 stamina, if you climb 1.2m you lose 2 stamina (1 for 1m and 1 for 0.2m) etc.
#
# You start by standing on first obstacle in list.
#
# Given a stamina number and a list of obstacles write a function that returns how many
# obstacles you can pass.
#
# Examples
...
def climb(stamina, obstacles):
    ...
    for i in range(0, len(obstacles) - 1):
        if obstacles[i] > obstacles[i + 1] and stamina > 0:
            height = obstacles[i] - obstacles[i + 1]
            if height <= stamina:
                if isinstance(height, float):
                    height = int(height + 1)
                    stamina = stamina - height
                    count = count + 1
                else:
                    stamina = stamina - height
                    count = count + 1
        elif obstacles[i] < obstacles[i + 1] and stamina > 0:
            height = obstacles[i] + obstacles[i + 1]
            if isinstance(height, float):
                height = int(height + 1)
                height = 2 * height
