# It takes 21 seconds to wash your hands and help prevent the spread of COVID-19.
#
# Create a function that takes the number of times a person washes their hands per day N
# and the number of months they follow this routine nM and calculates the duration in minutes
# and seconds that person spends washing their hands.
#
# Examples
# wash_hands(8, 7) ➞ "588 minutes and 0 seconds"
#
# wash_hands(0, 0) ➞ "0 minutes and 0 seconds"
#
# wash_hands(7, 9) ➞ "661 minutes and 30 seconds"
# Notes
# Consider a month has 30 days.

def solution(no_of_times, no_of_months):
    # result_in_seconds= ((no_of_times * 21)*30)*no_of_months
    # print(result_in_seconds)
    in_minutes = ((no_of_times * 21) * 30) * no_of_months / 60
    in_minutes_string = str(in_minutes)
    value = in_minutes_string.split(".")
    print(f"result is {value[0]} minutes and {value[1]} seconds")


if __name__ == "__main__":
    no_of_times = 7
    no_of_months = 9
    solution(no_of_times, no_of_months)
